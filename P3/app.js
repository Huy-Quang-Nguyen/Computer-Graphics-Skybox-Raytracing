...
function createWorld()
{
    ...
    for(var x = 0; x <= 20; x+=5) {
        for (var z = 0; z <= 20; z+=5) {
            x = getRndInteger(-50, 50);
            z = getRndInteger(-50, 50);
            m = new THREE.Mesh(
                new THREE.BoxGeometry(5, 5, 5),
                new THREE.MeshBasicMaterial({color: getRndColor(), side: THREE.DoubleSide}));
    ...
    function newhit(obj) {
        if  (obj != hit){
            if(hit != null){
                ...
                document.getElementById("destroyed").innerHTML++;
            }
        }
        if(obj != null){
